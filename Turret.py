...
while True:
        ...
        for i in range(4):
                ...
                try:
                  ser=serial.Serial(channel,9600,timeout=0.2,write_timeout=0.2)
                  connected = True;
                except:
                        continue
                else:
...
ser.flushInput()
ser.flushOutput()
ser.write("*".encode()) # Arduino will break from setup loop
...
while True:
        ...
        if(ser.in_waiting < 1):
                continue
        # we use this to remove message from serial collector
        print(ser.readline())
        while True:
                ...
                # if the `q` or `c` key is pressed, break from the loop
